...
public class Menu {
    ...
    private ArrayList<String> gravaAutomato = new ArrayList<>();
...
//        }
    ...
    public void gravaAFD() {
            
            for (int i=0; i<cs.afds.size();i++){
                this.gravaAutomato.addAll(cs.afds.get(i).getAutomatoArq());
            }
            try {
            Formatter arquivo = new Formatter("imprimeAutomato.txt");
 
                System.out.println("grava saida");

                for (int j=0; j < this.gravaAutomato.size(); j++){
                    arquivo.format("" + this.gravaAutomato.get(j) + "\n");
